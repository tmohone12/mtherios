import { json, type RequestHandler } from '@sveltejs/kit';
import { contextCheckpointRevertRequestSchema } from '$lib/contracts/memory';
import { executeLegacyEngineCommand } from '$lib/server/engine/routeCompatibility';
import { apiError, readJson } from '$lib/server/memory/http';

export const POST: RequestHandler = async (event) => {
	try {
		if (!event.params.id) return json({ error: 'Missing story id.' }, { status: 400 });
		if (!event.params.checkpointId) return json({ error: 'Missing checkpoint id.' }, { status: 400 });
		const request = await readJson(event, contextCheckpointRevertRequestSchema.partial());
		return json(await executeLegacyEngineCommand({
			storyId: event.params.id,
			command: 'context.checkpoint.revert',
			args: {
				...request,
				checkpointId: event.params.checkpointId,
			},
		}));
	} catch (error) {
		return apiError(error);
	}
};
